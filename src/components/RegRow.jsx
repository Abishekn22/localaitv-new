...
function RegRow({ c, constituency, setConst, setSearch, isLive, onSelect }) {
  const isSelected = constituency === c.en;
  const handleSelect = () => {
    if (onSelect) { onSelect(c); return; }
    setConst(c.en);
    setSearch('');
  };
  return (
    <div onClick={handleSelect} style={{padding:'10px 12px',borderBottom:`1px solid ${T.border}`,cursor:'pointer',background:isSelected?`rgba(208,2,27,0.12)`:'transparent',display:'flex',alignItems:'center',gap:8}}>
