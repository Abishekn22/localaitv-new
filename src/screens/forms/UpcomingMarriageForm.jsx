...
function UpcomingMarriageForm({ onBack }) {
  ...
  async function handleSubmit() {
    ...
    try {
      // ── Step 1: upload selected card / video to S3 (via /api/upload-file)
      //    before the JSON submit, so the backend can persist the resulting
      //    S3 keys in marriages.photo_s3_keys.
      const filesToUpload = [inviteFile, inviteVideo].filter(Boolean);
      const photoUris = filesToUpload.length
        ? await uploadPhotos(filesToUpload, reqId, 'marriage')
        : [];

      // ── Step 2: send the marriage metadata, including the URLs that the
      //    upload step returned (handler stores them in photo_s3_keys).
      const res = await fetch(`${API}/marriage-requests`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          request_id: reqId, location_id: getUserLocationId(), type: 'upcoming',
