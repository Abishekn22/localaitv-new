...
function CarSalesForm({ onBack }) {
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch(`${API}/vehicle-sales`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ request_id: reqId, make, model, year,
          km_driven: km, color, fuel_type: fuelType, price, description, contact, status: 'Pending Review' }),
      });
      setSuccess(reqId);
    } catch (e) {
